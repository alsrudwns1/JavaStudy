 ...
 * 를 일컫는 것으로, 메인쓰레드가 종료되도 계속 실행되며 Dead 상태가 되어야만
 * 실행이 종료되는 쓰레드를 말한다. 해당 쓰레드는 main()이 종료된 이후에도 
 * 계속 실행되는 것을 볼 수 있다.
 */
class NormalThread extends Thread {
	...
	public NormalThread() {}
	/*
	 * Thread 클래스의 생성자 호출 시 전달하는 인수를 통해 해당 쓰레드의
	 * 이름을 지정 가능. 이때 지정된 이름은 getName()을 통해 반환받을 수 있다.
	 */
	public NormalThread(String title) {
		super(title);
	}
	/*
	 * 쓰레드 동작 중 비정상적으로 종료될 때 InterruptedException이 
	 * 발생될 수 있으므로, 예외처리를 해야한다. 여기서는 즉시 처리하지 않고
	 * throw 해서 호출한 위치로 예외인스턴스를 던지기 한다.
	 * (예외를 발생한 지점에서 처리하지 않고 호출한 지점으로 던지기)
	 */
	void threadMethod() throws InterruptedException {
		for(int i=1; i<=10; i++) {
			/*
			 * 10번 반복중 2초씩 block상태로 들어가므로, 총 20초간 반복된다.
			 * 반복횟수와 쓰레드명을 출력한다. 
			 */
...
/*
 * 데몬쓰레드(Daemon Thread. 종속쓰레드 라고도 한다)
...
class DaemonThread extends Thread {
	 ...
	 * 3초에 한번씩 block 상태로 들어가게 구현되어 있다.
	 */
	@Override
	public void run() {
		while(true) {
			//쓰레드명을 getName()으로 출력
			System.err.printf("[쓰레드명:%s] Jazz가 흘러요~%n", getName());
			try {
				sleep(3000);
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		 ...
		 * 5로 지정된다. 쓰레드 인스턴스 생성 시 이름을 부여하는 방법으로 아래
		 * 두가지가 있다.
		 */
		
		//방법 1: 생성자를 통해 쓰레드의 이름 부여
		NormalThread yt1 = new NormalThread("1st 스레드");
		...
		/*
		 * 두 번째 독립쓰레드 생성 및 시작. 쓰레드명을 부여하지 않았으므로
		 ...
		 * 데몬쓰레드로 만들어준다. 즉 클래스 정의 시 차이는 없다
		 */
	...
	//Thread 클래스의 정적메서드를 통해 쓰레드의 여러 상태를 확인할 수 있다.
