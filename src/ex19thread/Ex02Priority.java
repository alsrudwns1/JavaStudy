...
class MessageSendingThread extends Thread {
	...
	public void run() {
		/*
		 * 각 쓰레드는 1000번씩 반복출력한다. 이때 부여받은 우선순위를 출력.
		 */
		for(int i=1; i<=1000; i++) {
			System.out.println(message + i + "(" + getPriority() + ")");
			/*
			 * 기본적으로 우선순위가 높은 쓰레드가 먼저 실행되지만 sleep()를
			 * 호출하면 실행 중 잠깐씩 블럭상태로 전환되어 우선순위가 낮은
			 * 쓰레드도 실행될 수 있는 기회가 생기게 된다.
			 */
			try {
				sleep(1);
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		/*
		 * 1단계 실행: 우선순위를 부여하지 않은 상태로 쓰레드 인스턴스를
		 * 생성하면, 동일한 우선순위 5를 부여받게 된다. 이때는 CPU가 알아서
		 * 작업을 분배해준다.
		 */
		...
		/*
		 * 2단계 실행: Thread 클래스의 정적상수를 이용해 우선순위 부여.
		 * 순서대로 10, 5, 1의 우선순위를 부여받게 된다. 상수 대신 정수로
		 * 부여해도 동일하다.
		 */
