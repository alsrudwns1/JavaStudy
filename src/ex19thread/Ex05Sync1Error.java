...
class SumMulti {
	...
	/*
	 * 스레드의 run()메서드에서 호출하는 메서드로 매개변수로 전달되는 n을
	 * 멤버변수에 누적해서 더한다. 여기서는 2개의 쓰레드가 5000번씩
	 * 각각 호출하고 있다.
	 */
...
class MultiAdderThread extends Thread {
	...
	/*
	 * 쓰레드를 통해 반복적으로 공유인스턴스의 addNum()을 호출한다.
	 * 이 경우 동기화 처리가 되지 않아 동시에 접근되는 경우가 발생하게되어
	 * 잘못된 결과가 출력된다.
	 */
	public void run() {
...
public class Ex05Sync1Error {
	...
	public static void main(String[] args) {
		/*
		 * 하나의 인스턴스를 두개의 쓰레드가 공유해서 사용하는 형태로 동작한다.
		 * 각각의 쓰레드가 5000번씩 반복해서 누적합을 계산하게 된다.
		 */
