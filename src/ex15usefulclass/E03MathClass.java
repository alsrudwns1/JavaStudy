...
/*
 * 자바의 규칙: java.lang 패키지 하위의 모든 클래스는 자동으로 import 된다.
 * 또한 정의하는 모든 클래스는 object를 상속받게 되어있다.
 * 따라서 java.lang 패키지의 모든 클래스는 별도의 선언 없이 사용 가능하며
 * objcect에서 제공하는 메서드는 오버라이딩이 가능
 */
...
public class E03MathClass extends Object {
	...
	public static void main(String[] args) {
		 ...
		 * -메서드는 static으로 선언되어있어 인스턴스 생성 없이 클래스명으로 호출가능
		 */
		...
		/*
		 * round의 경우 가장 가까운 정수 쪽으로 올림/버림을 결정한다.
		 * x.5이상이면 올림처리, 미만이면 버림처리를 한다.
		 * 따라서 3.4xxx 라면 버림처리를 한다.
		 */
		...
		/*
		 * 0~1사이의 실수값을 랜덤반환한다. 따라서 정수값을 만들고 싶다면
