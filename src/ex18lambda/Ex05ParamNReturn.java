...
/*
 * 매개변수는 있으나 반환값이 없는 추상매서드
 * 두개의 문자열을 전달받아 연결해 출력
 */
...
public class Ex05ParamNReturn {
	...
	public static void main(String[] args) {
		...
		/*
		 * 2개의 매개변수는 String 으로 추상메서드를 통해 유추할 수 있으므로
		 * 람다식에서는 생략할 수 있다. 매서드명을 제거한 후 괄호사이에
		 * 화살표를 삽입하여 람다식을 정의한다. 호출 시에는 인터페이스에 정의된
		 * 추상메서드를 이용하면 된다.
		 */
		StringConcat sConcat = (a,b) -> {
			System.out.println(a + " " + b);
		};
		sConcat.makeString(s1, s2);
		
		/*
		 * 매개변수가 없는 람다식을 정의해야하므로 빈 소괄호를 사용한다.
		...
		System.out.println("### 람다식 3");
		GugudanRange gugudanRange = (num1, num2) -> {
			//2차원 배열을 선언한 뒤 구구단을 각 인덱스에 삽입한다.
		...
		//람다식 호출 시 두 정수를 인수로 전달한 후 이차원배열을 반환받음
