...
public class Ex06Define2Supplier {
	...
	public static void main(String[] args) {
		//Supplier<T> : 공급자라는 의미를 갖고있다. 매개변수는 없고 리턴값 있는
		//get()메서드가 정의되어 있다. 주로 데이터를 생성해 리턴한다.
		
		/*interface Supplier<T> {
			T get();
		}
		여기서 타입매개변수 t는 추상매서드의 반환타입으로 지정됨 */
		
		//타입매개변수로 Integer클래스를 썼으므로 정수값을 반환받는 용도로
		//정의하겠다는 의미이다. 0~99사이의 난수를 생성한 뒤 반환하는
		//람다식으로 정의되어 있다.
		Supplier<Integer> sup = () -> {
			Random random = new Random();
			return random.nextInt(100);
		};
		//추상메서드인 get()을 통해 람다식 호출
		int rNum = sup.get();
		System.out.println("생성된 난수=" +rNum);
		
		//난수를 생성하는 람다식 sup와 정수 10을 인수로 전달함
		List<Integer> list = makeRandomNum(sup, 10);
		System.out.println("print문으로 출력");
		System.out.println(list);
		
		// list를 foreach문을 통해 출력하시오
		...
		// list를 Iterator를 통해 출력하시오
		System.out.println("iterator를 통해 출력");
