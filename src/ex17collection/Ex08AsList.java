...
public class Ex08AsList {
	...
	public static void main(String[] args) {
		/*
		 * string 인스턴스 배열을 생성과 동시에 초기화한다. 이 경우에는
		 * 초기데이터를 통해 크기를 유추할 수 있으므로 크기는 선언문장에서
		 * 생략해야 한다.
		 */
		String[] strArr = new String[] {"마린", "파이어뱃", "메딕" , "마린" };
		
		// Arrays.asList() : 일반 배열을 List<T> 컬렉션으로 변경할 때 사용.
		//단 값을 참조만 할 수 있고 입력이나 삭제는 불가능함
		List<String> list = Arrays.asList(strArr);
		
		listPrint("출력1", list);
		//현 상태에서는 데이터 추가 불가능. 런타임 에러가 남.
		list.add("탱크");//에러남
		
		//ArrayList<E>의 생성자를 통해 앞의 인스턴스를 복사하면 데이터 변경 가능.
		...
		/*
		 * set<e>컬렉션은 중복을 허가하지 않으므로 List에 저장된
		 * 중복값을 제거 가능하다.
		 */
