...
class FriendInfoHandler {
	...
	//컬렉션은 제네릭을 기반으로 하므로 인스턴스 생성 시 저장할 타입을 정하게 됨.
	//여기서는 Friend 클래스를 상속한 High, Univ를 저장할 것이니 이와같이 선언.
...
//		numOfFriends = 0;
	...
	public void addFriend(int choice) {
		...
		//별도로 사용할 필요가 없다. 단지 add()매서드를 통해 추가하면 된다.
	...
	public void showAllData() {
		//일반 for문을 사용하는 경우 변수 i를 인덱스로 써야 하므로
	...
	public void showSimpleData() {
		//확장 for문은 인덱스를 쓰지 않으므로 간단히 인스턴스를 인출 가능.
		for(Friend fr: lists) {
			fr.showBasicInfo();
		}
