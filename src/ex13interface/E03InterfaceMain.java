 ...
 * 
 * -자바는 단일 상속을 원칙으로 하지만, 인터페이스를 통해 다중상속을 구현 가능.
 * 
 * - 추상클래스와 동일하게 상속을 목적으로 제작되므로 구현받은 하위
 * 클래스에서는 반드시 추상메서드를 오버라이딩 해야함
 * 
...
interface MyInterface1 {
	...
	//public static final 은 생략하는것이 좋다. 
...
/*
 * 인터페이스는 콤마를 이용해 여러개의 인터페이스를 구현할 수 있다.
...
class MyClass extends SimpleClass implements MyInterface1, MyInterface2 {
	 ...
	 * 하면 된다. 오버라이딩은 항상 자식쪽의 메서드가 호출되므로 
	 * 아무런 문제도 발생하지 않음.
	 */
	@Override
	...
	/*
	 * SimpleClass 클래스의 멤버메서드는 필요한 경우에만 오버라이딩 하면 됨.
	 * 필수사항이 아니므로 에러가 발생하진 않음.
	 */
}
