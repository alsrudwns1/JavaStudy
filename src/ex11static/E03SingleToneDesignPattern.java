...
class SingleTone {
	int shareVar;
	/*
	 * 정의한 클래스와 동일한타입으로 선언된 정적 멤버변수로 JVM에 의해
	 * 프로그램이 시작될 때 미리 Method 영역에 로드되어 사용준비를 마치게 된다.
	 */
	private static SingleTone single = new SingleTone();
	
	/*
	 * 생성자를 private으로 선언하면 외부접근이 안되므로 new를 통해 인스턴스 생성 불가.
	 * 하지만 클래스 내부에선 호출가능.
	 */
	private SingleTone() {}
	
	//정적메서드로 선언된 메서드를 통해 해당 인스턴스의 참조값을 외부로 반환
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		/*
		 * 일반적인 방식의 인스턴스 생성이므로 new로 생성할 때마다 새로운
		 * 참조값(주소값)을 할당받는다.
		 */
		 ...
		 * 생성자가 not visible 즉 보이지 않는다는 에러가 발생함.
		 */
		//SingleTone st1 = new SingleTone();
		
		/*
		 * 정적메서드로 정의된 이 함수를 호출하여 이미 생성되어 있는
